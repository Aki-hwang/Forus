...
3. 지역별 검색어는 `src/lib/adQueries.ts` 에서 관리하며, 강남·명동·홍대를 **권역으로 넓게**(강남=강남/신사/압구정/역삼/청담, 명동=명동/을지로/충무로/중구, 홍대=홍대/합정/상수/연남/망원) 잡고, 각 검색어를 **송출국가로 확장**(일본인 타겟=한국+일본, 중국인 타겟=한국+대만, 한국인=한국)해 **검색어 70개를 매 수집마다 전부 훑습니다**(주차 로테이션 없음, 결과 7일 캐시). 일본인 광고는 한국뿐 아니라 **일본에서 송출되는 한국 클리닉 광고**까지 잡아야 누락이 없습니다. 각 URL에 `start_date[min]=(30일전)` 을 붙여 최근 광고를 우선 수집합니다
...
| `APIFY_PER_QUERY` | 검색당 최대 수집(과금) 건수 (최소 10) — 70쿼리×40≈2,800건/수집 | `40` |
...
```
src/
  auth.ts                  Auth.js v5 설정 (Google, JWT 세션)
  app/
    page.tsx               메인 대시보드
...
Google 소셜 로그인을 사용합니다. 코드는 준비되어 있고 **환경변수만 채우면** 동작합니다.
...
4. 배포 시 위 값들을 **Railway Variables** 에 등록하고, `AUTH_URL` 을 운영 도메인(예: `https://forus-advertising.up.railway.app`)으로 지정

> 관련 코드: `src/auth.ts`(설정), `src/app/api/auth/[...nextauth]/route.ts`(엔드포인트), `src/app/login/page.tsx`(로그인 페이지). Railway 등 Vercel 외 호스팅 대응으로 `trustHost: true` + `AUTH_URL`(운영 도메인) 을 사용합니다.
