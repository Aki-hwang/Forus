// /tw/[treatment] — 시술 가이드 (본체: ConsumerTreatmentPage)
...
export function generateStaticParams(): Params[] {
  return TREATMENT_GUIDES.tw.map((g) => ({ treatment: g.slug }));
}
...
export async function generateMetadata({
  ...
  return g ? treatmentMetadata("tw", g) : {};
}
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  const g = guideBySlug("tw", treatment);
  if (!g) notFound();
  return <ConsumerTreatmentPage locale="tw" g={g} />;
}
