...
//   APIFY_PER_QUERY    (선택) 검색당 최대 수집(과금) 건수, 기본 40 (최소 10)
...
// 중국어에서만 쓰는 문자 — 일본어(신자체·상용한자) 텍스트에는 등장하지 않는 것만 엄선.
//   간체자(일본 신자체와 코드포인트가 다른 것): 们这说时对动经门问长风东车书让爱还进过达边
//     开关头买卖见觉谁语请谢钱个为么样儿别场韩肤术发脸龄
//   중문 조사·구어(간체·번체 공통): 很你您咱啥逛吃呢吧吗嘛啊哦
//   번체 전용(일본 현대 표기에 없음): 們這麼妳嗎說讓錢覺來沒灣
// 가나 없는 한자 본문에서 이 문자가 1자라도 보이면 중국어 확정 — 검색 힌트(jp)보다 우선.
const CN_ONLY_CHARS =
  /[们这说时对动经门问长风东车书让爱还进过达边开关头买卖见觉谁语请谢钱个为么样儿别场韩肤术发脸龄很你您咱啥逛吃呢吧吗嘛啊哦們這麼妳嗎說讓錢覺來沒灣]/;
...
export function inferLang(text: string, hint?: "jp" | "kr" | "cn" | "en"): Lang {
  ...
  // (한글 해시태그 몇 개 섞인 중국어 광고가 KR로 새는 것도 함께 방지)
  if (han > hangul) {
    if (CN_ONLY_CHARS.test(text)) return "CN";
    return hint === "jp" ? "JP" : "CN";
  }
...
export async function fetchAdsViaApify(
  force = false,
  ...
  const maxQueries = Math.max(1, opts.maxQueries || Number(process.env.APIFY_MAX_QUERIES) || 24);
